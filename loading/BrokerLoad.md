...
可以使用的通配符有```? * [] {} ^```，[通配符使用规则参考](https://hadoop.apache.org/docs/stable/api/org/apache/hadoop/fs/FileSystem.html#globStatus-org.apache.hadoop.fs.Path-)。
    ...
    例如：
    1. 通过 "hdfs://hdfs_host:hdfs_port/user/data/tablename/\*/\*" , 可以匹配tablename下所有分区内的所有文件；
    2. 通过 "hdfs://hdfs_host:hdfs_port/user/data/tablename/dt=202104\*/\*" , 可以匹配tablename下所有4月分区的所有文件；
